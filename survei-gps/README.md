...
| `2026-07-26_jalur-tracking-tibu-sendalem.gpx` | 2026-07-26 | Jalur trekking dari dekat Kantor Desa sampai Air Terjun Tibu Sendalem, ~1 jam 24 menit, 894 titik dalam 1 segmen. 25 titik terakhir (stasioner, kecepatan rendah) dipakai sebagai centroid koordinat presisi titik wisata Tibu Sendalem di seeder — cuma selisih ~37,5m dari hasil decode Plus Code G64V+GR9 sebelumnya (jadi saling mengonfirmasi akurat). Akses jalur: tidak bisa dilalui mobil, wajib motor trail/mountain bike. | Sudah dikonversi ke GeoJSON dan tampil di peta aplikasi (lihat di bawah), sekaligus dipakai update koordinat titik wisata Tibu Sendalem. Belum diupload ke OpenStreetMap. |
| `2026-07-23_jalur-purekmas-bawak-goak.gpx` | 2026-07-23 | Jalan kaki dari sekitar Kantor Desa, terbagi 2 kaki (ada jeda GPS ±2 menit di antaranya): kaki pertama berakhir ~57,5m dari titik Bawak Goak Rivercamp (pintu tiket kawasan Bawak Goak/Taman Miring/Bukit Khesari), kaki kedua berakhir ~122,6m dari Purekmas (pintu tiket Purekmas). Dikonfirmasi user. Rekaman ke-2 dalam file yang sama (`2026-07-26_14-41_Sun.gpx`, menuju ke arah dusun) sengaja TIDAK diproses/ditambahkan atas permintaan user. | Sudah dikonversi ke GeoJSON (2 Feature terpisah) dan tampil di peta aplikasi (lihat di bawah). Belum diupload ke OpenStreetMap. |
| `2026-07-29_jalur-undak-telu.gpx` | 2026-07-29 | Rute dari Kantor Desa (naik motor lalu jalan kaki, ~83 titik) menuju kawasan Tibu Jam, Tibu Pancor, dan Undak Telu di sebelah timur desa — rutenya melewati persis lokasi Tibu Pancor di tengah jalan (±13m). File terpisah `2026-07-29_tibu-jam.gpx` dan `2026-07-29_tibu-pancor.gpx` dipakai untuk menentukan koordinat presisi 2 titik itu (centroid titik berkecepatan rendah), dikonfirmasi user sebagai kolam alami mirip mata air. Titik akhir rute (centroid 5 titik terakhir) dipakai untuk koordinat Undak Telu, dikonfirmasi user sebagai tangga air. Sebelumnya dua file GPX pendek ini juga sempat dicek terpisah dari 2 file `file_purekmas.gpx` & `file_bawak_goak.gpx` yang ternyata duplikat/tumpang tindih dengan jalur yang sudah ada, jadi tidak diproses ulang. | Rute utama sudah dikonversi ke GeoJSON dan tampil di peta aplikasi (lihat di bawah), koordinat 3 titik wisata baru (Tibu Jam, Tibu Pancor, Undak Telu) sudah masuk seeder. Belum diupload ke OpenStreetMap. |
| `2026-08-02_jalur-pancor-lima.gpx` | 2026-08-02 | Rute dari sekitar pintu masuk desa (naik motor lalu jalan kaki, 105 titik) menuju Pancor Lima di Dusun Temas Lestari — lokasi baru, ~600-900m dari titik wisata lain yang sudah ada. Titik akhir rute (centroid 2 titik terakhir, kecepatan terendah) dipakai untuk koordinat Pancor Lima, dikonfirmasi user sebagai mata air/kolam alami. | Sudah dikonversi ke GeoJSON dan tampil di peta aplikasi (lihat di bawah), koordinat titik wisata Pancor Lima sudah masuk seeder. Belum diupload ke OpenStreetMap. |
